...
if not ZOCALO_RECIPE.is_file():
    sys.exit(f"Error: Could not find recipe file {ZOCALO_RECIPE}")
...
for i in range(len(barcodes)):
    ...
    logger.info(f"Processing XChem plate barcode {barcode}, proposal {proposal_ref}")
    ...
    if drop != 2 or drop != 3:
        logger.info(
            f"Number of subwell images in {imagefolder} not does not correspond to a 2 or 3 drop plate"
        )
    ...
    protein_id = resp["proteins"][0][
        "proteinId"
    ]  # not sure how to link to exact protein id
    ...
    shipment_id = resp["items"][0][
        "shippingId"
    ]  # should make this empty too to avoid confusion?
    ...
    if drop == 3:
        ...
        container_type_id = (
            14  # 20 - MRC 2 drop, 14 - CrystalQuickX or 39 if new plate added
        )
    ...
    r = requests.post(
        url + f"/api/containers/{container_id}/inspections", headers=headers, json=body
    )
    ...
    for j in range(len(imagepaths)):
        ...
        r = requests.post(
            url + f"/api/containers/{container_id}/samples", headers=headers, json=body
        )
